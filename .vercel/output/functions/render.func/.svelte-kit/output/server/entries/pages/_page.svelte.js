...
<div class="${"home-container"}"><div class="${"flex flex-col items-center justify-center pointer-events-none"}">
		<h2${add_attribute("class", `text-3xl md:text-4xl lg:text-5xl font-bold transition-1s-in-out ${"md:initial-slide-up"}`, 0)}>Creating a Brighter Future
		</h2>
		<span${add_attribute("class", `w-[90%] h-[2px] bg-accent rounded-full my-8 transition-1s-in-out ${"md:initial-expand"}`, 0)}></span>
		<p${add_attribute("class", `text-xl md:text-2xl transition-1s-in-out ${"md:initial-slide-up"}`, 0)}>Pateros Catholic School is more than just a school - it&#39;s a life experience. Our unique
...
<div class="${"w-full h-[50vh] bg-accent relative"}"><div class="${"curve-white"}"><svg data-name="${"Layer 1"}" xmlns="${"http://www.w3.org/2000/svg"}" viewBox="${"0 0 1200 120"}" preserveAspectRatio="${"none"}"><path d="${"M0,0V7.23C0,65.52,268.63,112.77,600,112.77S1200,65.52,1200,7.23V0Z"}" class="${"shape-fill"}"></path></svg></div></div>
